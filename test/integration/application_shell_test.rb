...
class ApplicationShellTest < ActionDispatch::IntegrationTest
  ...
  test "スキップリンクの文言をリクエストのロケールで表示する" do
    ...
    assert_select "a.skip-link", skip_link_label(:en)
    assert_not_includes response.body, skip_link_label(:ja)
  end

  test "フッターの説明をリクエストのロケールで表示する" do
    get "/ja"

    assert_includes response.body, footer_description(:ja)
    assert_not_includes response.body, footer_description(:en)

    get "/en"

    assert_includes response.body, footer_description(:en)
    assert_not_includes response.body, footer_description(:ja)
  end
