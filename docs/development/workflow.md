...
検査が実際に実行されることは、完全検証から 2 段構えで保証する。

- `scripts/verify-security` が、呼び出しと binstub の中身を**実行行の完全一致**で検査する。
  部分文字列の一致では、`false && ...` や行末コメントのように実行されない書き方を
  正規の呼び出しと誤認する
- `scripts/verify-security-self-test` が、変異を加えて上記が拒否できることを確認する。
  加えて、外部コマンドをスタブへ置き換えた一時環境で `bin/verify` を実行し、
  各検査が実際に呼ばれること、失敗がそのまま非ゼロ終了へ伝播することを確認する

静的な検査だけでは「そう書かれていること」しか保証できない。
