...
完全検証は、`bin/verify` が定められた正規形を維持し、各検査の呼び出しと失敗伝播が
退行していないことを、構造検査と隔離実行プローブで検出する。

- `scripts/verify-security` が、`bin/verify` の**コメントを除いた実行行の並び全体**と、
  binstub の実行行を完全一致で検査する。
  個々の検査行だけを見る方式は使わない。行単位の判定では前後の shell 制御構造を
  解釈できず、複数行にまたがる短絡評価のように、必要な行を保ったまま
  条件付きで検査を飛ばす書き方を見抜けない
- `scripts/verify-security-self-test` が、変異を加えて上記が拒否できることを確認する。
  加えて、外部コマンドをスタブへ置き換えた隔離環境で `bin/verify` を実行し、
  各検査が実際に呼ばれること、失敗がそのまま非ゼロ終了へ伝播することを確認する。
  この実行では、プローブ専用の制御変数を検査対象へ渡さず、実行元の環境変数も引き継がない。
  プローブ実行を識別できると、プローブ中だけ検査を実行する実装を検出できなくなる

静的な検査だけでは「そう書かれていること」しか分からない。
逆に実行時の確認だけでは、正規の書き方から外れた実装を拒否できない。

このため `bin/verify` は、各検査を共通関数などの間接層を挟まずトップレベルで直接実行する。
実行を共通関数へ委ねると、その関数を無処理にするだけで全検査を飛ばせてしまう。
`bin/verify` を変更するときは、`scripts/verify-security` の正規形も同じ Pull Request で更新する。

これは退行の検出を目的とした仕組みであり、検証器と自己テストを同時に書き換える
意図的な改変まで防ぐものではない。
...
**現在、下限を明記しているシステムライブラリはない。**

以前は libvips について 8.13 以上を要求していた。CVE-2026-66066 の対策が
その版を必要としたためである。
[ADR 0064](../decisions/0064-no-image-variant-processing.md) で variant を
生成しないと決め、gem 側の backend もイメージの libvips も外したため、
要求する対象そのものが無くなった。

版の下限が不要になったライブラリは、検査ごと消さずに不在の検査へ置き換える。
消すだけにすると、ベースイメージの変更や別パッケージの依存で戻ったときに、
気付ける場所が無くなる。
完全検証は現在、Node.js・npm、画像処理のライブラリ、
PostgreSQL のクライアント（psql・pg_dump・pg_restore）の不在を実測している。

PostgreSQL のクライアントは、app 側から呼ぶ経路が無いため落とした。
接続は pg gem が libpq を直接呼び、バックアップと復元は db コンテナ側で実行する。
落とし過ぎを止めるため、libpq5 が**存在すること**も対で検査している。
不在の検査だけを並べると、必要なものまで消えたときに気付けない。
