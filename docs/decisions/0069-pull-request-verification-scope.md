...
- Pull Request の検証は 165 秒から 56 秒になる（実測）。
  テスト job が律速で、静的解析 job は 19 秒で返る

  | 区間 | 時間 |
  | --- | --- |
  | service コンテナの取得と起動 | 14 秒 |
  | checkout と Ruby の準備 | 7 秒 |
  | `db:prepare` | 2 秒 |
  | `rails test` | 27 秒 |

  ここから先を削るには、テストの件数か、
  PostgreSQL の版がローカルと一致することのどちらかを捨てることになる。
  どちらも CI の構成の問題ではないため、このタスクでは扱わない
